// # START OF Approval Card V2 - Beautiful card for approval waiting list
// Purpose: Display approval item in a colored card with status-based theming
// Features: Hover effect, truncated lists, responsive, gapoktan sub-groups
// Mobile-first approach with clean aesthetic design
...
import { truncateList, formatTanggal } from "@/lib/data/approval-mock";
...
const ApprovalCardV2: React.FC<ApprovalCardProps> = ({ item, onReview }) => {
  const statusConfig = APPROVAL_STATUS_CONFIG[item.status];

  const poktanNames = item.tipe === 'gapoktan'
    ? item.poktanList.map((p) => p.nama)
    : [];
  const truncatedPoktan = truncateList(poktanNames, 2);
  const truncatedKomoditas = truncateList(item.komoditas, 2);
  const truncatedOpt = truncateList(item.opt, 2);
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-4 space-y-3">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
          ...
          <span className="truncate">
            Diajukan oleh <strong className="text-gray-800 dark:text-gray-200">{item.diajukanOleh.nama}</strong>
          </span>
        </div>
      ...
      <div className="px-4 pb-4 pt-0">
        <Button
          ...
          size="sm"
        >
          <Icon icon="heroicons:clipboard-document-check" className="w-4 h-4 mr-1.5" />
          Review Pengajuan
        </Button>
      </div>
    </div>
